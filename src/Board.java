...
public class Board extends JPanel implements ActionListener, MouseListener {
	...
	private String[] icons = {"red.png", "purple.png", "yellow.png", "wall.png"};// the different images used for the tiles
	...
	private boolean addBlue = false;// used to determine when an ant has or hasn't been added to the board
	...
	private JButton fullBoard = new JButton("Full Board");
	...
	public Board() {
		...
		buttonPane.setLayout(new GridLayout(1, 1));// sets the layout for the buttonPane JPanel pane
		...
		if (result && results != null && resultCount < results.size()) {// checks if results are ready to be displayed
			if (timeCount++ % 75 == 0) {// staggers the evolution of the display of the search path results
				if (nodes[results.get(resultCount).getToNode()].getCost() == 3) {// checks for grass tiles on the path
					...
					nodes[results.get(resultCount++).getToNode()].setImage(new ImageIcon("redPath.png").getImage());// changes image to display path
				} else if (nodes[results.get(resultCount).getToNode()].getCost() == 4) {// checks for swamp tiles on the path
					//playSound(pathSound);// plays sound associated with path nodes
					nodes[results.get(resultCount++).getToNode()].setImage(new ImageIcon("purplePath.png").getImage());// changes image to display path
				} else if (nodes[results.get(resultCount).getToNode()].getCost() == 1) {// checks for terrain tiles on the path
					//playSound(pathSound);// plays sound associated with path nodes
					nodes[results.get(resultCount++).getToNode()].setImage(new ImageIcon("yellowPath.png").getImage());// changes image to display path
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == randomize) {// checks for cases where the randomize button has been pressed
			...
			if (nodes[255].getCost() != -1) {// checks whether board is complete
				...
				addBlue = false;// flag to indicate if ant is on the board
	...
	public void randomize() {
		...
		if ( (!addBlue && blankBoard) || !blankBoard ) {// checks whether an ant needs to be added
			...
			nodes[start] = new Node(point, new ImageIcon("index.png").getImage(), 0);// sets ant at the node
			addBlue = true;// sets flag to indicate the ant has been added
		}// if (ant)
		...
		if ( (!addDonut && blankBoard) || !blankBoard ) {// checks whether a donut needs to be added
			...
			nodes[goal] = new Node(point, new ImageIcon("green.png").getImage(), 0);// sets donut at the node
	...
	public void fullBoard() {
		...
		nodes[start] = new Node(point, new ImageIcon("index.png").getImage(), 0);// sets ant at last tile
		
		goal = 0;// sets goal to first tile
		point = nodes[goal].getTile();// gets coords of first tile
		nodes[goal] = new Node(point, new ImageIcon("green.png").getImage(), 0);// sets donut at last tile
	}// fullBoard() method
	...
	public void execute() {
		...
		pathDetails.setText("Path: " + start + "->" + results + " Cost: " + cost + " User Cost:" + userCost);// displays path details
	}// execute() method
	...
	public void mousePressed(MouseEvent e) {
		...
		if (position.x < 640 && position.y < 640) {// determines if the click is within the bounds of the game space
			...
			if (!addBlue && blankBoard && !active) {// checks if an ant needs to be added
				start = x * WIDTH_TILES + y;// gets the node number and sets it to start
				nodes[x * WIDTH_TILES + y] = new Node(point, new ImageIcon("index.png").getImage(), 0);// adds ant at specified node
				pathDetails.setText("Please click where you'd like to set the donut.");// displays instructions to the user
				addBlue = true;// sets flag to indicate ant added
				...
				nodes[x * WIDTH_TILES + y] = new Node(point, new ImageIcon("green.png").getImage(), 0);// adds donut at specified node
				...
				if (nodes[x * WIDTH_TILES + y].getCost() == 1) {// if tile is yellow
					nodes[x * WIDTH_TILES + y] = new Node(point, new ImageIcon("yellowUserPath.png").getImage(), 1);// set node to grass
                    userCost += 1;
				} else if (nodes[x * WIDTH_TILES + y].getCost() == 3) {// if tile is red
					nodes[x * WIDTH_TILES + y] = new Node(point, new ImageIcon("redUserPath.png").getImage(), 3);// set node to swamp
                    userCost += 3;
				} else if (nodes[x * WIDTH_TILES + y].getCost() == 4) {// if tile is purple
					nodes[x * WIDTH_TILES + y] = new Node(point, new ImageIcon("purpleUserPath.png").getImage(), 4);// set node to obstacle
                    userCost += 4;
				}// if (tile)
			}// if (ant)
		}// if (on board)
